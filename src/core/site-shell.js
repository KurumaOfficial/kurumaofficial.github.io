...
function isAutoRouteRedirectImmunePath(pathname = window.location.pathname) {
    ...
    /* Routes that own their own page UX must never auto-redirect to a product detail —
     * even if a featured product has autoRouteRedirect=true. The locale root and the
     * /donate, /products, /admin routes all qualify. */
    return (
        /\/(?:ru|en|ua)\/donate\/?$/i.test(normalizedPath) ||
        /\/(?:ru|en|ua)\/products\/?$/i.test(normalizedPath) ||
