...
function drawGlowSparkle(context, x, y, radius, alpha, palette = 'gold', cachedGradient = null) {
    ...
    if (!gradient) {
        ...
        if (palette === 'platinum') {
            ...
            // Ruby palette (was warm yellow). Bright core → saturated ruby
            // → transparent accent. Matches the site-wide accent #8b1a2a /
            // accent-light #b52239 instead of the legacy gold-yellow that
            // clashed with the rest of the design.
            gradient.addColorStop(0, 'rgba(255,232,232,0.95)');
            gradient.addColorStop(0.35, 'rgba(220,80,100,0.55)');
            gradient.addColorStop(1, 'rgba(180,50,70,0)');
        }
    }
...
function createSupportCard(supporter, { rank = 0, top = false, locale = 'ru', effectsEnabled = true } = {}) {
    ...
    if (tier.kind === 'platinum') {
        ...
        // Ruby palette (mid tier): the more the donor gives, the deeper the
        // red glow becomes. RGB 139,26,42 is the site-wide --color-ruby and
        // 230,90,110 is its hot-side highlight. The variable names below
        // are kept (“gold” / “goldCount” / etc.) on purpose to avoid a
        // wide JS rename — the *visual* palette is what changed.
        borderAlpha = 0.12 + tier.glow * 0.78;
        boxShadow = [
            `0 0 ${8 + tier.glow * 24}px rgba(180,50,70,${0.06 + tier.glow * 0.22})`,
            `0 0 ${16 + tier.glow * 36}px rgba(180,50,70,${0.04 + tier.glow * 0.14})`,
            `0 0 ${24 + tier.glow * 58}px rgba(180,50,70,${0.025 + tier.glow * 0.08})`,
            `inset 0 0 ${6 + tier.glow * 16}px rgba(230,90,110,${tier.glow * 0.10})`,
        ].join(',');
        ringWidth = 1 + tier.glow * 2.5;
        ringAlpha = 0.15 + tier.glow * 0.85;
        ringGlow = `0 0 ${4 + tier.glow * 14}px rgba(180,50,70,${tier.glow * 0.7})`;
        backgroundAlpha = tier.aura * 0.55;
        card.style.cssText = `
            --g-alpha: ${(0.15 + tier.glow * 0.85).toFixed(3)};
            border-color: rgba(180,50,70,${borderAlpha.toFixed(3)});
            box-shadow: ${boxShadow};
            background: rgba(180,50,70,${backgroundAlpha.toFixed(4)});
        `;
    } else {
        // Standard tier (< $100): essentially transparent ruby tint, just
        // enough to nudge it apart from the surrounding section bg.
        borderAlpha = 0.12;
        boxShadow = [
            '0 0 8px rgba(180,50,70,0.05)',
            '0 0 16px rgba(180,50,70,0.03)',
            '0 0 24px rgba(180,50,70,0.02)',
            'inset 0 0 6px rgba(230,90,110,0)',
        ].join(',');
        ringWidth = 1;
        ringAlpha = 0.15;
        ringGlow = '0 0 4px rgba(180,50,70,0)';
        backgroundAlpha = 0;
        card.style.cssText = `
            --g-alpha: 0.150;
            border-color: rgba(180,50,70,${borderAlpha.toFixed(3)});
    ...
    avatar.append(
        createElement('div', {
            ...
            style: `box-shadow:${ringGlow};border:${ringWidth.toFixed(1)}px solid ${tier.kind === 'platinum' ? `rgba(190,225,255,${ringAlpha.toFixed(3)})` : `rgba(230,90,110,${ringAlpha.toFixed(3)})`};`,
            'aria-hidden': 'true',
        }),
    );
...
/**
 * Populate the footer social links on the donate route.
 * Uses the same social icons / platform list as the main site renderer.
 */
function renderFooterSocials(socials) {
    const container = document.getElementById('donateFooterSocials');
    if (!container) return;

    const PLATFORMS = [
        { key: 'youtube',  label: 'YouTube',  svg: '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.01 2.01 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.01 2.01 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31 31 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.01 2.01 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A100 100 0 0 1 7.858 2zM6.4 5.209v4.818l4.157-2.408z" fill="currentColor"/></svg>' },
        { key: 'discord',  label: 'Discord',  svg: '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M13.545 2.907a13.2 13.2 0 0 0-3.257-1.011.05.05 0 0 0-.052.025c-.141.25-.297.577-.406.833a12.2 12.2 0 0 0-3.658 0 8 8 0 0 0-.412-.833.05.05 0 0 0-.052-.025c-1.125.194-2.22.534-3.257 1.011a.04.04 0 0 0-.021.018C.356 6.024-.213 9.047.066 12.032q.003.022.021.037a13.3 13.3 0 0 0 3.995 2.02.05.05 0 0 0 .056-.019q.463-.63.818-1.329a.05.05 0 0 0-.01-.059l-.018-.011a9 9 0 0 1-1.248-.595.05.05 0 0 1-.02-.066l.015-.019q.127-.095.248-.195a.05.05 0 0 1 .051-.007c2.619 1.196 5.454 1.196 8.041 0a.05.05 0 0 1 .053.007q.121.1.248.195a.05.05 0 0 1-.004.085 8 8 0 0 1-1.249.594.05.05 0 0 0-.03.03.05.05 0 0 0 .003.041c.24.465.515.909.817 1.329a.05.05 0 0 0 .056.019 13.2 13.2 0 0 0 4.001-2.02.05.05 0 0 0 .021-.037c.334-3.451-.559-6.449-2.366-9.106a.03.03 0 0 0-.02-.019m-8.198 7.307c-.789 0-1.438-.724-1.438-1.612s.637-1.613 1.438-1.613c.807 0 1.45.73 1.438 1.613 0 .888-.637 1.612-1.438 1.612m5.316 0c-.788 0-1.438-.724-1.438-1.612s.637-1.613 1.438-1.613c.807 0 1.451.73 1.438 1.613 0 .888-.631 1.612-1.438 1.612" fill="currentColor"/></svg>' },
        { key: 'telegram', label: 'Telegram', svg: '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8.287 5.906q-1.168.486-4.666 2.01-.567.225-.595.442c-.03.243.275.339.69.47l.175.055c.408.133.958.288 1.243.294q.39.01.868-.32 3.269-2.206 3.374-2.23c.05-.012.12-.026.166.016s.042.12.037.141c-.03.129-1.227 1.241-1.846 1.817-.193.18-.33.307-.358.336a8 8 0 0 1-.188.186c-.38.366-.664.64.015 1.088.327.216.589.393.85.571.284.194.568.387.936.629q.14.092.27.187c.331.236.63.448.997.414.214-.02.435-.22.547-.82.265-1.417.786-4.486.906-5.751a1.4 1.4 0 0 0-.013-.315.34.34 0 0 0-.114-.217.53.53 0 0 0-.31-.093c-.3.005-.763.166-2.984 1.09" fill="currentColor"/></svg>' },
    ];

    const fragment = document.createDocumentFragment();
    PLATFORMS.forEach(({ key, label, svg }) => {
        const href = cleanUrl(socials?.[key] || '');
...
function boot() {
    ...
    renderFooterSocials(siteData.socials);
...
// Exposed for the temporary /test preview route. Not part of the public
// donate runtime — boot() is the only entry that the live route uses.
export { createSupportCard, getDonorStyle };
