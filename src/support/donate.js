...
const COPY = Object.freeze({
  ru: {
    ...
    introAfterRole: 'в нашем ',
    discordLabel: 'Discord-сервере',
    introTail: ' и ваш профиль будет на сайте — просто укажите свой Discord-логин при покупке или донате.',
    paymentsTitle: 'Способы поддержки',
    paymentsDesc: 'Ссылки и подписи задаются через админ-панель. Можно добавить сколько угодно кнопок.',
    ...
    buttonsEmpty: 'Кнопки оплаты пока не настроены. Добавьте ссылки через админ-панель.',
  ...
  en: {
    ...
    introAfterRole: 'role in our ',
    discordLabel: 'Discord server',
    introTail: ' and your profile will appear on the site — just include your Discord login when you donate or buy support.',
    paymentsTitle: 'Support methods',
    paymentsDesc: 'Links and labels come from the admin panel. You can add as many buttons as needed.',
    ...
    buttonsEmpty: 'Payment buttons are not configured yet. Add them in the admin panel.',
  ...
  ua: {
    ...
    introTail: ' і ваш профіль зʼявиться на сайті — просто вкажіть свій Discord-логін під час покупки або донату.',
    paymentsTitle: 'Способи підтримки',
    paymentsDesc: 'Посилання та підписи задаються через адмін-панель. Можна додати будь-яку кількість кнопок.',
    topTitle: 'Загальний топ 3',
    topDesc: 'Що більша сума, то сильніше золотисте сяйво картки.',
    supportersTitle: 'Підтримали нас',
    supportersEmpty: 'Поки ніхто не підтримав проєкт. Картки зʼявляться тут після перших донатів.',
    buttonsEmpty: 'Кнопки оплати ще не налаштовані. Додайте посилання через адмін-панель.',
...
function buildIntro(container, copy, minimumAmountUsd, discordUrl) {
  ...
  container.append(
    document.createTextNode(copy.introTail),
  );
}
...
function createPaymentCard(button, fallbackNote) {
  ...
  const attrs = {
    className: isDisabled ? 'pay-btn-disabled' : 'pay-btn',
  };
  ...
  card.append(
    createElement('span', { className: 'pay-btn-label', textContent: button.label || 'Поддержать' }),
...
function createSupportCard(supporter, { rank = 0, maxAmount = 0, top = false } = {}) {
  ...
  if (rank > 0) {
    card.append(
      createElement('div', { className: 'support-rank' }, `#${rank}`),
    );
  }
