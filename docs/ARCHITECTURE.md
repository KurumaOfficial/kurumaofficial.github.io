...
- `src/admin/editor.js` — состояние редактора, формы, черновик, localStorage, секретный вход и orchestration UI.
...
4. `Применить` переносит draft в preview-состояние страницы.
5. `Сохранить локально` записывает нормализованные данные в `localStorage`.
6. `Сохранить для всех` сначала загружает выбранные файлы товара в репозиторий, затем обновляет `src/data/site-data.js`.
